...
private[spark] class Client(
    ...
    sparkConf.get(AM_NODE_LABEL_EXPRESSION) match {
      case Some(expr) =>
        try {
          ...
          val method = amRequest
            .getClass.getMethod("setNodeLabelExpression", classOf[String])
          method.invoke(amRequest, expr)

          val setResourceRequestMethod = appContext
            .getClass.getMethod("setAMContainerResourceRequest", classOf[ResourceRequest])
    ...
    sparkConf.get(ROLLED_LOG_INCLUDE_PATTERN).foreach { includePattern =>
      try {
        ...
        sparkConf.get(ROLLED_LOG_EXCLUDE_PATTERN).foreach { excludePattern =>
          val setRolledLogsExcludePatternMethod = logAggregationContext
              .getClass.getMethod("setRolledLogsExcludePattern", classOf[String])
  ...
  private def verifyClusterResources(newAppResponse: GetNewApplicationResponse): Unit = {
    ...
    if (executorMem > maxExecutorMem) {
      throw new IllegalArgumentException(s"Required executor memory ($executorMemory" +
        ...
        "this cluster! Please check the values of 'spark.yarn.allocation.executor.maxMemory'.")
    }
    ...
    if (amMem > maxAmMem) {
      throw new IllegalArgumentException(s"Required AM memory ($amMemory" +
        ...
        "Please check the values of 'spark.yarn.allocation.am.maxMemory'.")
    }
  ...
  private def createConfArchive(): File = {
    ...
    if (sparkConf.getBoolean("spark.yarn.client.uploadHadoopConf.enabled", false)) {
  ...
  private def setupLaunchEnv(
    ...
    if (pythonPath.nonEmpty) {
      ...
      val pythonPathExecutorEnv = (sparkConf.getExecutorEnv.toMap.get("PYTHONPATH") ++
        pythonPathList).mkString(YarnSparkHadoopUtil.getClassPathSeparator)
  ...
  private def createContainerLaunchContext(newAppResponse: GetNewApplicationResponse)
    ...
    val amArgs =
      ...
      Seq("--properties-file", buildPath(YarnSparkHadoopUtil.expandEnvironment(Environment.PWD),
        LOCALIZED_CONF_DIR, SPARK_CONF_FILE)) ++
      Seq("--dist-cache-conf", buildPath(YarnSparkHadoopUtil.expandEnvironment(Environment.PWD),
        LOCALIZED_CONF_DIR, DIST_CACHE_CONF_FILE))
  ...
  private def findPySparkArchives(): Seq[String] = {
    sys.env.get("PYSPARK_ARCHIVES_PATH")
      ...
      .getOrElse {
        ...
        require(files != null, "PYSPARK_ARCHIVES_PATH or python/lib not found")
...
private object Client extends Logging {
  ...
  private[yarn] def populateClasspath(
    ...
    addClasspathEntry(
      buildPath(YarnSparkHadoopUtil.expandEnvironment(Environment.PWD), LOCALIZED_CONF_DIR,
