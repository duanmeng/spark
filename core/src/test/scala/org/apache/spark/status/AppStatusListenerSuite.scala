...
class AppStatusListenerSuite extends SparkFunSuite with BeforeAndAfter {
  ...
  test("force to evict old data") {
    val testConf = conf.clone()
      .set(MAX_RETAINED_JOBS, 2)
      .set(MAX_RETAINED_STAGES, 2)
      .set(UI_FORCECLEAN_ENABLED, true)
    val listener = new AppStatusListener(store, testConf, true)

    // Start 3 jobs, the first one should be evicted, though it's not stopped.
    time += 1
    listener.onJobStart(SparkListenerJobStart(1, time, Nil, null))
    listener.onJobStart(SparkListenerJobStart(2, time, Nil, null))
    listener.onJobStart(SparkListenerJobStart(3, time, Nil, null))
    assert(store.count(classOf[JobDataWrapper]) === 2)
    intercept[NoSuchElementException] {
      store.read(classOf[JobDataWrapper], 1)
    }
    // Start 3 stages, the first one should be evicted, though it's not stopped.
    time += 1
    val stages = Seq(
      new StageInfo(1, 0, "stage1", 4, Nil, Nil, "details1"),
      new StageInfo(2, 0, "stage2", 4, Nil, Nil, "details2"),
    ...
    // Start a task with stage(2, 0)
    time += 1
    val stage2 = store.read(classOf[StageDataWrapper], Array(2, 0))
    val task2 = createTasks(2, Array("1")).head
    listener.onTaskStart(SparkListenerTaskStart(stage2.info.stageId, stage2.info.attemptId, task2))
    assert(store.count(classOf[TaskDataWrapper]) === 1)

    // Start a task with stage which is dropped by event queue
    val task3 = createTasks(2, Array("1")).head
    listener.onTaskStart(SparkListenerTaskStart(999, 999, task3))
    assert(store.count(classOf[TaskDataWrapper]) === 2)

    // Start a task with stage(3, 0)
    val stage4 = store.read(classOf[StageDataWrapper], Array(3, 0))
    val task4 = createTasks(2, Array("1")).head
    listener.onTaskStart(SparkListenerTaskStart(stage4.info.stageId, stage4.info.attemptId, task4))
    assert(store.count(classOf[TaskDataWrapper]) === 3)

    // Start a new stage, stage(2, 0) and tasks without stage should be evicted
    val attempt = new StageInfo(3, 1, "stage2", 4, Nil, Nil, "details2")
    attempt.submissionTime = Some(time)
    listener.onStageSubmitted(SparkListenerStageSubmitted(attempt, new Properties()))
    assert(store.count(classOf[TaskDataWrapper]) === 1)
  }
